...
契约（`代码\数据准备\README.md` 一、二、三.3.7；《12-第5阶段任务书》§五 硬约束 3／5／7、§八）：

    输入  raw\{doc_id}.json          只读；raw\ 采集后不可修改（《12》§4.2）
...
《12》§八 修订后的监管公开信息口径：同题文档（行政处罚决定书等）先按"原标题（当事人）"构造
...
被淘汰的文档**不改动 raw\ 文件**：raw\ 是《12》§4.2 规定的不可变目录，因此去重结论只写
`reports\dedup_log.jsonl`，由 clean.py 读取后排除。（README §一 表格中"对 raw\ 打重复标记"
的写法与《12》§4.2 的不可变口径冲突，本实现按《12》与用户口径执行，已在交付报告中登记。）

依赖方向：正文指纹按 clean.py 的清洗结果计算、标题规范化复用 clean.py 的实现，从而使
clean\documents.jsonl 里 content_sha256_16 的唯一性由构造保证（《12》§八"正文 SHA-256 前
