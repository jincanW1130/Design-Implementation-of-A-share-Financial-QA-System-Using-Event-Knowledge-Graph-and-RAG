...
| `event_first.py` | T1′ | 网络（市场级检索） | 第 4.6节 的「事件类型优先」补样：市场级标题检索、按事件族排名挑公司、写证据到 `勘察\` |
| `v21_report.py` | 核对 | `数据集\v2.1\` | **只读**重算 v2.1 的全部对外数字，用于核对文档与报告，不产出数据集文件 |
...
### 4.6 事件类型优先的市场级补样（v2.1 定向补样，2026-09-25 增设）

前五阶段审核认定数据集撑不起本体（8 类事件里「重大合同」0 篇、9 条关系里 3 条无证据），
并建议「扩样的同时**按事件类型与公司间关系分层**」。v2.0 只做了规模扩张，v2.1 补上另一半。

**做法**：不先定公司再取公告，而是**先按事件类型在市场范围内检索，再按命中情况挑公司**。

1. 对 `POST /new/hisAnnouncement/query` 传 `searchkey=<关键词>`、**不传 `stock`**、`seDate` 取采集窗，
   得到**市场级**命中。注意：`column` 传 `szse` 或 `sse` 结果**完全相同**（同一份市场数据），
   两个都查会重复计数；也不要省略 `column`（会混入 H 股代码）。
2. 标题里带 `<em>`／`</em>` 高亮标签，必须先剥掉再做正则匹配。
3. 按「该公司在窗口内的去重命中数」降序排名，取**尚未在现有公司集里**的前若干家作为新增公司。
4. 新增公司只取**命中该事件族关键词**的公告，每家公司 ≤6 篇，仍走 level-2 等间距铺开；
   其余过滤（标题排除、6 万字符上限、判重）一律照旧。

**关键词集**（见 `config.EVENT_FIRST`）与两次修正：

* 重大合同族：`中标|重大合同|订单|框架协议|供货|签约|签订.*合同`
* 产品族：`临床试验|注册证|医疗器械注册|药品注册|上市许可|获批上市|新产品|首台|投产|量产|取得.*批件|获得.*批准`
  * **修正一（污染）**：产品族必须排除再融资污染——「向特定对象发行股票申请获得中国证监会同意注册批复」
    会命中「注册／获批」，但它属股权事件。判据：标题命中 `发行|股票|债券|募集|上市公告书|可转换` 即剔除。
  * **修正二（口径）**：新闻补样最初限定「提及 ≥2 家**新增**公司」，只得到 11 篇；改为
    「提及 ≥2 家**数据集内全部**公司」后为 53 篇（`company_list` 长度 ≥2 由 24 篇升到 66 篇）。
    后者才对应「公司间关系」的定义——关系建立在数据集内任意两家公司之间。

**脚本**：`event_first.py`（市场级检索、剥标签、按族排名、公司概况、`--probe` 写证据）、
`config.py` 的 `EVENT_FIRST` 块（关键词集、目标数、每公司篇数、时段配比、新闻口径与预算）、
`v21_report.py`（只读重算，用于核对下列数字）。构建证据与逐轮日志在 `代码\数据准备\勘察\`。

**v2.1 实测效果**（v2.0 → v2.1）：文档 456 → 709、文本块 3731 → 5018、
公司 50 → 105（沿用 50 加新增 55：35 家按重大合同族、20 家按产品族）；
公告标题命中重大合同族 **0 → 100**、产品族 **2 → 102**；
`company_list` 长度 ≥2 **13 → 66**、`subject_companies` 长度 ≥2 **1 → 9**。

**沿用不变**：字段集与 `subject_companies` 阈值、`doc_id` 分块与 `chunk_id = doc_id*1000 + chunk_index`、
切分 400／512／128／50、Embedding 模型与 revision、`data_cutoff_time`、三条排除规则、判重键。
**v2.1 只改「选谁、选哪些事件」，不改任何格式或参数。**
